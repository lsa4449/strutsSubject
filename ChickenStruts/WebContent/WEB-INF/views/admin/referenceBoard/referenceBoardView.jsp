...
$(function(){
    ...
    // 삭제
    $('#btn2').click(function(){
    	$(location).attr('href', '${pageContext.request.contextPath}/admin/referenceBoard/referenceBoardDelete.do?post_no=${postInfo.post_no}');
    })
   ...
   $('#btn3').click(function(){
    	...
    	$(location).attr('href', '${pageContext.request.contextPath}/admin/referenceBoard/referenceBoardReply.do'+ queryString + parentInfo); // 경로 지정 freeboardReplyFormURI(지정해준거) 와 queryString + parentInfo 
    }); 
...
<body>
<form name="refereneceViewForm" class="form-horizontal" role="form" 
		action="${pageContext.request.contextPath}/admin/referenceBoard/referenceBoardUpdate.do?post_no=${postInfo.post_no}" method="post">
	<div class="form-group">
		<label class="control-label col-sm-2" for="bo_title">제목:</label>
		<div class="col-sm-10">
			<input style="width:350px" type="text" class="form-control" id="bo_title" name="post_title" >
		</div>
	</div>
	<div class="form-group">
		<label class="control-label col-sm-2" for="bo_name">작성자 :</label>
		<div class="col-sm-10"> 
			<input style="width:350px" type="text" class="form-control" id="bo_name" name="bo_name" >
		</div>
	</div>
	<div class="form-group">
				...
				<c:forEach items="${postInfo.items }" var="fileitemInfo" varStatus="stat">
					...
					<c:if test="${stat.last }">
						...
						<img src="/files/${fileitemInfo.file_save_name }" alt="pic1"
							 onclick="javascript:location.href='${pageContext.request.contextPath}/admin/referenceBoard/referenceBoardFileDownload.do?fileName=${fileitemInfo.file_upload_name}';">
					</div>
				</c:forEach>
			</div> -
